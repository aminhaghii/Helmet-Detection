...
class DatasetUnifier:
    ...
    def _get_image_label_pairs(self, dataset_info: Dict, split: str) -> List[Tuple[Path, Path]]:
        """
        ...
        if split_path.name == 'images' or (split_path / 'images').exists():
            images_dir = split_path if split_path.name == 'images' else split_path / 'images'
                ...
                if len(parts) >= 5:
                    ...
                    if class_name:
                        ...
                        if class_name in self.unified_classes:
                            ...
                            logger.warning(f"Class {class_name} not found in unified classes")
                    else:
                        logger.warning(f"Class ID {old_class_id} not found in source classes")
    ...
    def _copy_and_convert_data(self, pairs: List[Tuple[Path, Path]], 
                              output_split_dir: Path, source_classes: Dict[int, str],
                              dataset_name: str):
        """
        ...
        for i, (img_path, label_path) in enumerate(tqdm(pairs, desc=f"Processing {dataset_name}")):
    ...
    def unify_datasets(self, train_ratio: float = 0.7, val_ratio: float = 0.2, 
                      test_ratio: float = 0.1, shuffle: bool = True) -> bool:
        """
        ...
        for dataset_info in self.datasets:
            ...
            for split in ['train', 'val', 'test']:
                ...
                for img_path, label_path in weighted_pairs:
                    all_pairs.append((img_path, label_path, source_classes, dataset_name))
                
                logger.info(f"Dataset {dataset_name} - {split}: {len(pairs)} samples (weight: {weight})")
        ...
        for split_name, pairs in splits.items():
            ...
            for img_path, label_path, source_classes, dataset_name in pairs:
                if dataset_name not in dataset_pairs:
                    dataset_pairs[dataset_name] = {'pairs': [], 'classes': source_classes}
                    ...
                    if split in config:
                        ...
                        if images_path.exists():
                            ...
                            if split_path.exists():
                                ...
                                logger.warning(f"Could not find {split} directory in {dataset_path}")
                                continue
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='unified_dataset',
