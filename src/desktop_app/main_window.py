...
class SafetyDetectionApp:
    ...
    def create_main_layout(self):
        ...
        # Video display label
        self.video_label = ctk.CTkLabel(self.video_frame, text="Camera Feed", font=("Arial", 16))
    ...
    def create_camera_controls(self):
        ...
        ctk.CTkLabel(camera_section, text="Camera Controls", font=("Arial", 14, "bold")).pack(pady=5)
        
        # Camera selection
        self.camera_var = tk.StringVar(value="0")
        ctk.CTkLabel(camera_section, text="Camera ID:").pack(anchor="w", padx=10)
        self.camera_entry = ctk.CTkEntry(camera_section, textvariable=self.camera_var, width=100)
        self.camera_entry.pack(pady=2, padx=10)
        
        # Resolution selection
        ctk.CTkLabel(camera_section, text="Resolution:").pack(anchor="w", padx=10, pady=(10, 0))
        self.resolution_var = tk.StringVar(value="640x480")
        resolution_combo = ctk.CTkComboBox(camera_section, 
        ...
        self.start_camera_btn = ctk.CTkButton(button_frame, text="Start Camera", 
                                            command=self.start_camera, width=120)
        self.start_camera_btn.pack(side="left", padx=2)
        
        self.stop_camera_btn = ctk.CTkButton(button_frame, text="Stop Camera", 
                                           command=self.stop_camera, width=120, state="disabled")
    ...
    def create_detection_controls(self):
        ...
        ctk.CTkLabel(detection_section, text="Detection Controls", font=("Arial", 14, "bold")).pack(pady=5)
        
        # Confidence threshold
        ctk.CTkLabel(detection_section, text="Confidence Threshold:").pack(anchor="w", padx=10)
        self.confidence_var = tk.DoubleVar(value=self.config['detection']['confidence_threshold'])
        self.confidence_slider = ctk.CTkSlider(detection_section, from_=0.1, to=1.0, 
                                             variable=self.confidence_var, number_of_steps=90)
        self.confidence_slider.pack(pady=2, padx=10, fill="x")
        
        self.confidence_label = ctk.CTkLabel(detection_section, text=f"Value: {self.confidence_var.get():.2f}")
        ...
        self.start_detection_btn = ctk.CTkButton(button_frame, text="Start Detection", 
                                               command=self.start_detection, width=120, state="disabled")
        self.start_detection_btn.pack(side="left", padx=2)
        
        self.stop_detection_btn = ctk.CTkButton(button_frame, text="Stop Detection", 
                                              command=self.stop_detection, width=120, state="disabled")
        self.stop_detection_btn.pack(side="right", padx=2)
        
        # Capture button
        self.capture_btn = ctk.CTkButton(detection_section, text="Capture Frame", 
    ...
    def create_model_controls(self):
        ...
        ctk.CTkLabel(model_section, text="Model Controls", font=("Arial", 14, "bold")).pack(pady=5)
        
        # Model status
        self.model_status_label = ctk.CTkLabel(model_section, text="Model: Not Loaded", 
                                             text_color="red")
        self.model_status_label.pack(pady=2)
        
        # Model buttons
        self.load_model_btn = ctk.CTkButton(model_section, text="Load Model", 
                                          command=self.load_model_dialog)
        self.load_model_btn.pack(pady=2)
        
        self.optimize_model_btn = ctk.CTkButton(model_section, text="Optimize Model", 
                                              command=self.optimize_model, state="disabled")
    ...
    def create_statistics_display(self):
        ...
        ctk.CTkLabel(stats_section, text="Statistics", font=("Arial", 14, "bold")).pack(pady=5)
    ...
    def initialize_model(self):
        ...
        try:
            ...
            model_files = [f for f in os.listdir(model_path) if f.endswith('.pt')] if os.path.exists(model_path) else []
        ...
        try:
            ...
            if self.is_detecting and self.inference_engine:
                ...
                display_frame = self.detection_display.draw_detections(
                    frame, self.detection_results, 
                    fps=self.performance_logger.get_fps()
                )
            else:
                display_frame = frame
                self.detection_results = []
    ...
    def load_model_dialog(self):
        ...
        filename = filedialog.askopenfilename(
            ...
            filetypes=[("PyTorch files", "*.pt"), ("ONNX files", "*.onnx"), ("All files", "*.*")]
        )
    ...
    def load_custom_model(self, model_path: str):
        ...
        try:
            ...
            # Update GUI
            self.model_status_label.configure(text=f"Model: {os.path.basename(model_path)}", 
                                            text_color="green")
            self.optimize_model_btn.configure(state="normal")
    ...
    def update_statistics(self):
        ...
        try:
            ...
            if self.detection_results:
                summary = self.detection_display.create_detection_summary(self.detection_results)
    ...
    def save_settings(self):
        ...
        try:
            ...
            settings_path = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'user_settings.yaml')
