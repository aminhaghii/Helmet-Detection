...
class DetectionDisplay:
    ...
    def _draw_overlay_info(self, 
        ...
        overlay = image.copy()
        cv2.rectangle(overlay, (10, 10), (300, overlay_y + line_height * 4), 
        ...
        if self.show_fps:
            ...
            cv2.putText(image, fps_text, (20, overlay_y), 
        ...
        cv2.putText(image, count_text, (20, overlay_y), 
                   self.font, self.font_scale, self.colors['text'], self.font_thickness)
        overlay_y += line_height
        
        # Draw safety status
        safety_status = self._get_safety_status(detection_count, stats)
        status_color = self.colors['safe'] if safety_status['is_safe'] else self.colors['alert']
        cv2.putText(image, f"Status: {safety_status['message']}", (20, overlay_y), 
        ...
        if stats and self.show_stats:
            if 'inference_time' in stats:
                ...
                cv2.putText(image, inference_text, (20, overlay_y), 
                           self.font, self.font_scale, self.colors['text'], self.font_thickness)
    ...
    def draw_alert_overlay(self, image: np.ndarray, alert_message: str, alert_type: str = 'warning') -> np.ndarray:
        """
        ...
        cv2.putText(image, alert_message, (text_x, text_y), 
    ...
    def create_detection_grid(self, images_with_detections: List[Tuple[np.ndarray, List[Dict]]], 
