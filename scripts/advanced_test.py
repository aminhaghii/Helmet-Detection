...
class ProfessionalHelmetDetector:
    ...
    def detect_helmets(self, frame):
        ...
        if results[0].boxes is not None:
            ...
            if has_no_helmet:
                ...
                self.consecutive_no_helmet = 0
                if (
                    time.time() - self.last_alert_time > 3
        ...
        if results.boxes is not None:
            ...
            for box, conf, cls in zip(boxes, confidences, classes):
                ...
                conf_size = cv2.getTextSize(
                    confidence_text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1
                )[0]
    ...
    def draw_professional_statistics_panel(self, frame):
        ...
        cv2.addWeighted(
            overlay, self.panel_alpha, frame, 1 - self.panel_alpha, 0, frame
        )
        ...
        detection_rate = (
            (self.total_detections / self.total_frames * 100)
        ...
        status_color = (
            self.colors["success"]
            if avg_fps > 20
            else self.colors["warning"] if avg_fps > 10 else self.colors["danger"]
        )
        ...
        metrics = [
            ...
            (
                "FPS",
                f"{avg_fps:.1f}",
                self.colors["success"] if avg_fps > 20 else self.colors["warning"],
            ),
        ...
        if self.alert_active:
            ...
            if int(time.time() * 3) % 2:
                ...
                cv2.putText(
                    frame,
                    "⚠ SAFETY ALERT: NO HELMET DETECTED!",
    ...
    def draw_mini_graph(self, frame, x, y, width, height, data, title, color):
        ...
        # Title
        cv2.putText(
            frame, title, (x + 5, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1
        )
        ...
        for i, val in enumerate(data):
            ...
            py = (
                ...
                + height
                - 5
                - int(((val - min_val) / (max_val - min_val)) * (height - 10))
            )
            points.append((px, py))
    ...
    def draw_professional_controls_info(self, frame):
        ...
        for i, (key, desc, color) in enumerate(controls):
            ...
            cv2.rectangle(
                frame,
                (control_x + 10, y_pos - 12),
                (control_x + 30, y_pos + 2),
                color,
                -1,
            )
            cv2.putText(
    ...
    def draw_header_info(self, frame):
        ...
        cv2.putText(
            frame,
            "🛡 HSE VISION - HELMET DETECTION SYSTEM",
            (15, 25),
            cv2.FONT_HERSHEY_DUPLEX,
            0.7,
            self.colors["accent"],
        ...
        # Separator line
        cv2.line(
            frame, (0, header_height), (w, header_height), self.colors["accent"], 2
        )
...
def main():
    ...
    try:
        while True:
            ...
            if show_stats:
                annotated_frame = detector.draw_professional_statistics_panel(
                    annotated_frame
                )
        ...
        avg_inference = (
            np.mean(detector.inference_times) if detector.inference_times else 0
        )
        avg_confidence = (
            np.mean(detector.confidence_history) if detector.confidence_history else 0
        )
